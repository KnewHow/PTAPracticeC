...
/**
 * A method implement two polynomials multiple,but we use add
 * algorithms
 */
ptrPoly multiply(ptrPoly a, ptrPoly b) {
	ptrPoly head = (ptrPoly) malloc(sizeof(struct node));
	ptrPoly p = a->next;
	...
	while (p) {
		...
		while (q) {
			 ...
			 */
			if (p->coefficient * q->coefficient != 0) {
				ptrPoly newNode = (ptrPoly) malloc(sizeof(struct node));
				newNode->exponents = p->exponents + q->exponents;
